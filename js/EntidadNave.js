...
/**
 * Esta entidad representa la nave del jugador, la �ltima oportunidad
...
function EntidadNave() {
    ...
    this.mover = function(delta) {
        //Si estamos en el margen izquierdo del mapa, no podemos movernos m�s a la izquierda
        if (this.dx < 0 && this.x < 10) {
            return;
        }
        //Si estamos en el margen derecho del mapa, no podemos movernos m�s a la derecha
        if (
            this.dx > 0 &&
            this.x > juego.getAnchuraCanvas() - (this.ancho + 10)
        ) {
            return;
        }
    ...
    /**
     * Notificaci�n de que la nave del jugador ha chocado con algo
     *
     * otro es la entidad con la que hemos chocado
     */
    this.colosionadoCon = function(otro) {
        //Si hemos chocado con una nave alien�gena, morimos y el juego se acaba
